...
function synthesizeTrajectory(moonPos) {
...
export async function registerRoutes(app) {
    ...
    app.get("/api/near-earth-objects", async (req, res) => {
        try {
            ...
            const hazardousAsteroids = asteroids.filter(asteroid => asteroid.is_potentially_hazardous_asteroid ||
                parseFloat(asteroid.close_approach_data?.[0]?.miss_distance?.kilometers || '0') < 10000000).slice(0, 3);
    ...
    app.get('/api/space-weather', async (req, res) => {
        ...
        const cached = getCache(cacheKey);
        if (cached)
            return res.json({ ...cached, fallback: false });
        try {
            const endDate = new Date().toISOString().split('T')[0];
            ...
            if (isRateLimited)
                res.setHeader('X-ODIN-RateLimit', 'true');
