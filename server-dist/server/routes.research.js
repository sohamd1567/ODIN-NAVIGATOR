...
router.post('/predict-hazards', async (req, res) => {
    ...
    if (!traj || !startDate || !endDate)
        return res.status(400).json({ error: 'traj, startDate, endDate required' });
    try {
...
router.post('/generate-backups', (req, res) => {
    const { traj } = req.body;
    if (!traj)
        return res.status(400).json({ error: 'traj required' });
...
router.post('/replay-trajectory', async (req, res) => {
    ...
    try {
        const hazards = await research.predictHazardsForTrajectory(traj, startDate, endDate);
        const failures = hazards.filter((h) => h.type === 'solar_flare' || h.type === 'neo');
        ...
        for (const f of failures) {
            ...
            if (correction)
                corrections.push({ hazard: f, correction });
...
router.post('/log-lesson', async (req, res) => {
    ...
    if (!missionId || !lesson)
        return res.status(400).json({ error: 'missionId and lesson required' });
    try {
