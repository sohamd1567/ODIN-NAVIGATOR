...
export async function writeLessons(json) {
    await ensureDataDir();
    await fs.writeFile(LESSONS_FILE, JSON.stringify(json, null, 2), 'utf8');
}
export async function logLesson(missionId, lesson) {
...
export async function getLessons(missionId) {
    const data = await readLessons();
...
export async function predictHazardsForTrajectory(traj, startDate, endDate) {
    const hazards = [];
    // Fetch solar flares
    try {
...
export function generateBackupsForTrajectory(traj) {
    const backup1 = { ...traj, name: (traj.name || 'traj') + ' - backup-delay', points: traj.points.map(p => ({ ...p, t: p.t + 60 })) };
    const backup2 = { ...traj, name: (traj.name || 'traj') + ' - backup-early', points: traj.points.map(p => ({ ...p, t: Math.max(0, p.t - 60) })) };
...
function circularVelocity(radiusKm) {
...
export function applyOdinCorrection(traj, anomaly) {
...
export async function composeBriefing(date, lat, lon, startDate, endDate) {
    const briefing = { solarFlareRisk: 'Unknown', neoCloseApproach: 'Unknown', moonIlluminationPct: null, fuelMargin: 'Unknown', narrative: [] };
    try {
